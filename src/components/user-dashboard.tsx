...
import { Clock, CheckCircle, XCircle, Upload, ImageIcon, FileText } from "lucide-react"
...
export function UserDashboard() {
  ...
  const verifiedCount = artworks.filter((art) => art.status === "verified").length
  const pendingCount = artworks.filter((art) => art.status === "pending").length
  const rejectedCount = artworks.filter((art) => art.status === "rejected").length
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress className="h-2" value={(verifiedCount / totalCount) * 100} />
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <Progress className="h-2" value={(pendingCount / totalCount) * 100} />
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <Progress className="h-2" value={(rejectedCount / totalCount) * 100} />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 grid-cols-1">
        <Card className="col-span-1">
          <CardHeader>
            ...
            <CardDescription>Your recently submitted artwork for verification</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {artworks.map((artwork) => (
                <div key={artwork.id} className="overflow-hidden rounded-lg border bg-white dark:bg-gray-950 shadow-sm">
                  <div className="aspect-square relative">
                    ...
                    />
                    <div className="absolute top-2 right-2">{getStatusBadge(artwork.status)}</div>
                  </div>
                  <div className="p-4">
                    <h3 className="font-medium truncate">{artwork.title}</h3>
                    <p className="text-sm text-gray-500">Submitted: {formatDate(artwork.dateSubmitted)}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Register your artwork on the blockchain</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col items-center justify-center py-6">
            ...
            <p className="text-sm text-center text-gray-500 mb-6 max-w-md">
              Upload your artwork images and details to begin the verification process
            </p>
          </CardContent>
          <CardFooter>
            <Link href="/app/upload" className="w-full">
              <Button className="w-full bg-teal-600 hover:bg-teal-700">Upload Artwork</Button>
            </Link>
          </CardFooter>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Check authenticity of any registered artwork</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col items-center justify-center py-6">
            ...
            <p className="text-sm text-center text-gray-500 mb-6 max-w-md">
              Scan a QR code or enter a token ID to verify the authenticity of any artwork
            </p>
          </CardContent>
          <CardFooter>
